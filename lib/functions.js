...
module.exports.deepstate2cot = (item, stale, ptime) => {
    const dt = Date.now();
    const dtD = new Date(dt).toISOString();
    const dtDs = new Date(dt + (stale * 1000) + 21600000).toISOString(); // stale time = creation time + stale time parameter + 6h
    const prodTime = new Date(ptime).toISOString();

    if (typeof item.properties.name === 'undefined') return;

    let nameFull = item.properties.name.split("///");
    console.log("nameFull",nameFull);
    let nameEn = (typeof nameFull[1] !== 'undefined') ? nameFull[1].replace(/(\r\n|\n|\r)/gm, "") : nameFull[0].replace(/(\r\n|\n|\r)/gm, "");
    if (/\s/g.test(nameEn[0])) nameEn = nameEn.substring(1);
    
    if ( (nameEn != "Direction of attack") && (nameEn != "")) {
        ...
        else if (nameID.includes("spetsnaz")) type = "a-h-F";
        else if (nameID.includes("engineer")) type = "a-h-G-U-C-E";
        else if (nameID.includes("cruise")) type = "a-h-S-C-L-C-C";
        else if (nameID.includes("reconnaissance")) type = "a-h-G-U-C-R";
        else if (nameID.includes("intelligence")) type = "a-h-G-U-U-M";
